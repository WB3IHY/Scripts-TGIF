#!/bin/bash
#########################################################
#  Nextion TFT Support for Nextion 2.4 & 3.5" 		#
...
#########################################################
# Valid Screen Names for VE3ZRD - NX3224K024, NX4832K035
...
if [ -d "$homedir"/Nextion_Temp ]; then
  	sudo rm -R "$homedir"/Nextion_Temp
    	...
    	if [ "$scn" == "NX3224K024" ]; then
	  	sudo git clone --depth 1 https://github.com/VE3ZRD/NX3224K024 "$homedir"/Nextion_Temp
		tst=1
	fi     
