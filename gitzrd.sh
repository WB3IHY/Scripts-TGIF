...
#	echo "Function VE3ZRD"
 ...
 #      echo "Screen = $scn"
	...
	if [ "$scn" == "NX4832K035" ]; then
		cleandirs
	  	sudo git clone --depth 1 https://github.com/TGIF-Network/NX4832K035-ZRD "$homedir"/Nextion_Temp
	...
	if [ "$scn" == "NX4024K032" ]; then
 		cleandirs
                sudo git clone --depth 1 https://github.com/TGIF-Network/NX4024K032-ZRD "$homedir"/Nextion_Temp
                sudo chmod +x "$homedir"/Nextion_Temp/*.sh
                mkdir /usr/local/etc/Nextion_Support
                sudo rsync -avqru "$homedir"/Nextion_Temp/* /usr/local/etc/Nextion_Support/ --exclude=NX*
                sudo cp "$homedir"/Nextion_Temp/"$model$tft" /usr/local/etc/
	...
	if [ "$scn" == "NX3224K024" ]; then
 		cleandirs
                sudo git clone --depth 1 https://github.com/TGIF-Network/NX3224K024-ZRD "$homedir"/Nextion_Temp
                sudo chmod +x "$homedir"/Nextion_Temp/*.sh
                mkdir /usr/local/etc/Nextion_Support
                sudo rsync -avqru "$homedir"/Nextion_Temp/* /usr/local/etc/Nextion_Support/ --exclude=NX*
                sudo cp "$homedir"/Nextion_Temp/"$model$tft" /usr/local/etc/
